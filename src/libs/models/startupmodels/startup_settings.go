...
func (c *startupSettings) CreateRevision(ctx context.Context, startupSettingId flake.ID, input *coresSdk.UpdateStartupSettingInput, revisionId *flake.ID) (err error) {
	stmt := `
		INSERT INTO startup_setting_revisions(startup_setting_id, token_name, token_symbol, token_addr, wallet_addrs, type, vote_token_limit, vote_assign_addrs, vote_support_percent, vote_min_approval_percent, vote_min_duration_hours, vote_max_duration_hours, proposer_type, proposer_token_limit, proposal_support_percent, proposal_min_approval_percent, proposal_min_duration, proposal_max_duration)
		VALUES (${startupSettingId},${tokenName},${tokenSymbol},${tokenAddr},${walletAddrs},${type},${voteTokenLimit},ARRAY[${voteAssignAddrs}],${voteSupportPercent},${voteMinApprovalPercent},${voteMinDurationHours},${voteMaxDurationHours},${proposerType},${proposerTokenLimit},${proposalSupportPercent},${proposalMinApprovalPercent},${proposalMinDuration},${proposalMaxDuration})
		RETURNING id;
	`
	...
	query, args := util.PgMapQueryV2(stmt, map[string]interface{}{
		...
		"{proposalSupportPercent}": input.ProposalSupportPercent,
